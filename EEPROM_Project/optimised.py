...
# Optimized Integrated Operations with Logging and Automatic Run
...
def dump_eeprom(start=0, length=32):
    ...
    if start < 0 or end_address > EEPROM_SIZE:
...
def write_bytes(start_address, data_list):
    """Write multiple bytes sequentially (optimized)."""
    end_address = start_address + len(data_list)
    if start_address < 0 or end_address > EEPROM_SIZE:
        print("Address out of range!")
        return
    ...
    with open(EEPROM_FILE, "r+b") as f:
        for offset, val in enumerate(data_list):
            ...
            if write_cycles[addr] >= MAX_WRITE_CYCLES:
                print(f"Max write cycles reached at address {addr}, write ignored.")
                log_action(f"WRITE_IGNORED address={addr} value={val:#04X}")
                continue
            f.seek(addr)
            f.write(bytes([val & 0xFF]))
            write_cycles[addr] += 1
    save_write_cycles()
    print(f"Wrote {len(data_list)} bytes starting at address {start_address}")
    log_action(f"WRITE_BLOCK start={start_address} length={len(data_list)}")


def read_bytes(start_address, length):
    """Read multiple bytes sequentially (optimized)."""
    ...
    for offset, val in enumerate(data):
        print(f"Read from addr {start_address + offset}: {val:02X}")
        log_action(f"READ address={start_address + offset} value={val:02X}")
...
# -------------------------------------------------------------------------
# Interactive menu or automatic run with input logging
# -------------------------------------------------------------------------
if __name__ == "__main__":
    init_eeprom()

    interactive_log = []  # Store all user inputs
        ...
        while True:
            ...
            if choice == "1":
                try:
                    ...
                    write_byte(addr, val)
                    interactive_log.append(f"Write single byte - Address: {addr}, Value: {val}")
                except ValueError:
                ...
                try:
                    addr = int(input("Address (0-1023): "))
                    read_byte(addr)
                    interactive_log.append(f"Read single byte - Address: {addr}")
                except ValueError:
                    print("Invalid input! Enter integers only.")
                ...
                try:
                    ...
                    interactive_log.append(f"Write multiple bytes - Start address: {addr}, Values: {byte_list}")
                except ValueError:
                    print("Invalid input! Enter integers only.")
                ...
                try:
                    ...
                    interactive_log.append(f"Read multiple bytes - Start address: {addr}, Length: {length}")
                except ValueError:
                    print("Invalid input! Enter integers only.")
                ...
                try:
                    ...
                    interactive_log.append(f"Write string - Start address: {addr}, Text: '{text}'")
                except ValueError:
                    print("Invalid input! Enter integers only.")
                ...
                try:
                    ...
                    interactive_log.append(f"Read string - Start address: {addr}, Length: {length}")
                except ValueError:
                    print("Invalid input! Enter integers only.")
                ...
                try:
                    ...
                    interactive_log.append(f"Dump EEPROM section - Start: {start}, Length: {length}")
                except ValueError:
                    print("Invalid input! Enter integers only.")
                ...
                try:
                    ...
                    interactive_log.append(f"Compute checksum - Start: {start}, Length: {length}")
                except ValueError:
                    print("Invalid input! Enter integers only.")

            elif choice == "0":
                print("Exiting EEPROM simulation.")
                # Log all interactive inputs
                log_action("=== Interactive session inputs ===")
                for entry in interactive_log:
                    log_action(entry)
                log_action("=== End of interactive session ===")
                break
